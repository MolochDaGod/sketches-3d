 ...
 *
 * Portal travel keeps `?toon=` + session physics. F8 edits gravity / MM / jump / dash
 * on the same Ammo player (sceneConf + controller setters).
 */
...
/** Hub Destiny-mobility defaults from `notes/game-testing.md`. */
...
const rememberToon = (pick: { gender: NexusGender; id: string }) => {
  try {
    sessionStorage.setItem(TOON_SESSION_KEY, `${pick.gender}:${pick.id}`);
  } catch {
    /* private mode */
  }
};
...
export function readNexusPlayPhysics(): NexusPlayPhysics | null {
  try {
    ...
    return {
      ...
      jumpVelocity: Number.isFinite(parsed.jumpVelocity)
        ? parsed.jumpVelocity!
        : NEXUS_PLAY_DEFAULTS.jumpVelocity,
      dashMagnitude: Number.isFinite(parsed.dashMagnitude)
        ? parsed.dashMagnitude!
        : NEXUS_PLAY_DEFAULTS.dashMagnitude,
    };
  } catch {
    return null;
  }
...
export function writeNexusPlayPhysics(p: NexusPlayPhysics) {
  try {
    sessionStorage.setItem(PHYS_SESSION_KEY, JSON.stringify(p));
  } catch {
    /* */
  }
}

/** Keep `?toon=` across SvelteKit portal navigations. Physics lives in sessionStorage. */
export function nexusPortalGoto(path: string) {
  const pick = parseNexusToonQuery(typeof location !== 'undefined' ? location.search : '');
  rememberToon(pick);
  const href = `${resolve(path)}?toon=${encodeURIComponent(`${pick.gender}:${pick.id}`)}`;
...
export function applyNexusPlayPhysics(viz: Viz, p: NexusPlayPhysics) {
  ...
  conf.player.dashConfig = {
    ...DefaultDashConfig,
    ...(conf.player.dashConfig ?? {}),
    enable: true,
    dashMagnitude: p.dashMagnitude,
  };
  const fp = viz.fpCtx;
  if (!fp) return;
  ...
  fp.playerController.setDashConfig(
    dash.enable,
    dash.dashMagnitude,
    dash.minDashDelaySeconds,
    dash.useExternalVelocity ?? false
  );
}
...
const mountF8Editor = (viz: Viz, sceneConf: SceneConfig) => {
  ...
  sceneConf.customControlsEntries = [
    ...(sceneConf.customControlsEntries ?? []),
    { key: 'f8', label: 'Mobility editor', action: toggle },
  ];
  viz.registerDestroyedCb(close);
};

/**
 * Same play body + F8 mobility on every physics scene. Call after sceneLoader,
 * before BulletPhysics reads sceneConf.
 */
...
export async function hydrateNexusPlay(viz: Viz, sceneConf: SceneConfig, sceneDef: SceneDef) {
  ...
  if (phys) {
    if (!sceneConf.player.moveSpeed) {
      sceneConf.player.moveSpeed = { onGround: phys.onGround, inAir: phys.inAir };
    }
    ...
    sceneConf.player.dashConfig = {
      ...
      ...(sceneConf.player.dashConfig ?? {}),
      enable: true,
      dashMagnitude: phys.dashMagnitude,
    };
  }
  ...
  if (wantBody && !sceneConf.player.mesh) {
    try {
      sceneConf.player.mesh = await loadNexusPlayBody(viz, {
