...
export class BulletPhysics {
  ...
  public startMainGameTick = () => {
    ...
    this.packStateBufPtr = this.Ammo._malloc(36);
  ...
  /**
   * Drains events for things like colliding with jump pads, dash tokens, etc. and calls the appropriate handlers.
   */
  ...
  public resetPlayerState = () => {
    ...
    // full reset of as much player state as possible to initial state after level load
  ...
  public assertInitialState = () => {
    ...
    // packed layout: [0-2] pos, [3-5] extVel, [6] vertVel,
    //                [7] flags (u32 bitcast), [8] floorUserIndex (i32 bitcast)
    const extVelX = heap[base + 3];
    const extVelY = heap[base + 4];
    const extVelZ = heap[base + 5];
    const vertVel = heap[base + 6];
    const dv = new DataView(heap.buffer, this.packStateBufPtr, 36);
    const flags = dv.getUint32(7 * 4, true);
    const onGround = (flags & 1) !== 0;
    const isJumping = (flags & 2) !== 0;
    const floorUserIndex = dv.getInt32(8 * 4, true);
